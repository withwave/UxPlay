...
### 한 번 걸리면 재시작 전까지 그 영상이 재생되지 않던 문제

`/play`는 항목을 만들면서 uuid를 **먼저** 기록하고, 그 다음 리버스 채널로 마스터 플레이리스트를
요청한다. 그 사이에 TEARDOWN이 들어오면 클라이언트는 답하지 않고 **uuid만 있고 내용은 빈 항목**이
남는다. 이후 같은 영상의 `/play`는 전부 `get_playlist_by_uuid`에 매칭돼 "이미 받아둔 것"으로
취급되고 조기 return하므로 FCUP을 건너뛴다 — 내용은 영영 채워지지 않고 로컬 HLS 서버는 계속 404,
파이프라인은 READY에서 멈춘다. **프로세스가 사는 동안 그 영상은 영구히 막힌다.**

측정 (로그):
```
79407: POST /play
79520: fcup_request: send sent Request of 1048 bytes
79612: TEARDOWN                     ← 응답 전
80439: POST /play → FCUP 없음 → 404
81921: POST /play → FCUP 없음 → 404
```

재사용 전에 `get_master_playlist()`가 실제로 있는지 확인하고, 없으면 항목을 버리고 제대로 받아온다.
**이 경로는 아직 실기에서 발동한 적이 없다** — 재현하려면 FCUP 발신과 응답 사이에 TEARDOWN이 들어와야
한다.
