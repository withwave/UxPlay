...
- **`Switch to HLS video`** — 맥과 함께 들어온 항목이라 Windows 쪽 배선도 그 커밋에 포함돼 있었다. 끄면 기능 비트가 빠진 채로 재광고되어 클라이언트가 영상 URL을 넘기지 않고 미러 화면 안에서 재생한다. 토글하면 서비스를 내렸다 올리므로 연결된 클라이언트가 끊길 수 있다
...
둘 다 `uxplay-disconnect`로 합류시켰다. OSD 쪽은 창에 `WM_CLOSE`를 보내 같은 경로로 들어온다.

**OSD 닫기 버튼은 화면이 멈춰도 살아 있어야 한다.** 오버레이는 프레임 위에 그려지므로 프레임이 끊기면 마지막 프레임이 컨트롤을 담은 채 화면에 남는데, 히트테스트를 좌우하는 페이드는 프레임이 아니라 타이머로 돈다. 그대로 두면 **버튼이 보이는데 눌리지 않는** 상태가 된다 — 곤란한 세션을 끝내라고 있는 버튼으로서는 최악이다. 마지막 그리기 시각을 기록해 1초 넘게 프레임이 없으면 화면을 정지 화면으로 보고, 그때는 알파와 무관하게 닫기 버튼을 받는다. 볼륨·스크러버 등 나머지는 원래 규칙 그대로다(안 보이는 표적이 시크나 볼륨을 건드리는 편이 아무 일도 안 하는 것보다 나쁘다).

### 세션마다 되돌려야 할 상태는 세션 시작에서 되돌려라

`fullscreen_applied`은 "이번 세션에서 이미 전체화면을 걸었다"는 표시다. 이것을 클라이언트 teardown 콜백에서만 되돌리게 두면 새는데, **그 콜백은 항상 오지 않는다.** 창을 X로 닫는 것은 우리 쪽에서 세션을 끝내는 것이고, 사라지는 클라이언트는 teardown을 보내지 않는다. 놓칠 때마다 플래그가 남아 다음 스트림이 메뉴는 켜진 채로 창 모드로 뜬다 — 재현이 들쭉날쭉해 원인을 찾기 어려운 종류다.

`video_renderer_init()`에서 되돌린다. 렌더러가 만들어진다는 것이 곧 스트림이 시작된다는 뜻이고, 그것이 "연결 시"의 정의다. 맥은 싱크가 세션마다 새로 만들어지고 그때 `start-fullscreen`을 읽으므로 반대 방향에서 같은 자리에 도달한다.
